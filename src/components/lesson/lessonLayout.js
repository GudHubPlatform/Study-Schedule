...
export default function getHtml() {
    const { title, teacher } = this.lesson;
    ...
    return /*html*/ `
        <div class="container">
            ...
            <div class="${contentContainerClass.replace('.', '')}">
                ...
                <span class="${lessonTeacherNameClass.replace('.', '')} ${
                    teacher ? '' : noTeacherClass.replace('.', '')
                }">
                ${teacher ? teacher : '???'}
                </span>
...
const shortText = text =>
    text.length > 11 && [...text][text.length - 1] !== '.' ? [...[...text].splice(0, 11), '...'].join('') : text;
